...
\captionof{figure}{Andamento dei prezzi per il titolo TESLA/DOLLARO, accompagnato da due medie mobili a diverso periodo: fast\_ma a 20 giorni mentre slow\_ma a 100 giorni. Le frecce verdi e rosse indicano i segnali di buy e sell ricavati dalle intersezioni fra le medie.}\label{Figura 2}
	\end{center}
\end{fig}
...
\begin{fig}
	...
	\captionof{figure}{\\~\\Schema semplificato di Backpropagation Neural Nework. Una Neural Network è un'interconnessione di un gruppo di nodi chiamati neuroni. Ha un livello di neuroni di input, un numero variabile di livelli intermedi (\textit{hidden layers}) e un livello di output. I segnali si propagano dagli input attraverso gli strati nascosti, fino all'output. Nel caso di backpropagation, durante il training, i pesi, e gli altri parametri della rete sono modificati propagando l'errore dall'output verso gli input.}\label{Figura 6}
\end{fig}
...
\captionof{figure}{\\~\\Estrazione di conoscenza sugli eventi: basandosi sulle notizie di giornale e avendo coscienza dell'impatto che possono avere certi eventi sull'economia, è possibile stimare se un certo evento può influire positivamente o negativamente sul mercato.}\label{Figura 9}
\end{fig}
...
In questo documento, tuttavia, si fa riferimento a "Sentyment" intendendo l'applicazione sviluppata, e non la divisione AI di Nexid.\\~\\\\~\\
	...
	\captionof{figure}{\\~\\Architettura di Sentyment. Nei riquadri sono mostrati i quattro principali componenti, suddivisi al loro interno in ulteriori sotto-moduli. Il grafico è diviso in \textit{front-end} e \textit{back-end}, che rappresentano rispettivamente le interfacce esposte all'utente utilizzatore del sistema, e la parte di logica non accessibile dall'esterno}\label{Figura 10}
\end{fig}
\\~\\
...
\begin{fig}
	...
	\end{center}
	\captionof{figure}{Grafico comparativo dei risultati prodotti da 10 differenti AI di Sentyment. È analizzata la funzione guadagno (\textit{gain}, Formula (3)). Questo frammento di dataset, che fa riferimento ai prezzi di BITCOIN-EURO di fine 2019, è particolarmente rappresentativo e compare spesso all'interno dell'intero documento. Si nota come le AI abbiano tutte un comportamento diverso, ma ciascuna con pattern simili: nella settimana del 24 ottobre, a inizio grafico, tutte quante prevedono un aumento del prezzo del titolo e investono capitale, per poi riscontrare un grosso guadagno rappresentato dalla ripida pendenza che tutte le AI mostrano. Seguono altre situazioni simili, in cui le varie linee del guadagno mostrano delle "colline" simili per ciascuna AI.}\label{Figura 12}
\end{fig}
...
\begin{fig}
	\begin{center}
		...
		\end{tabular}
	\end{center}
	\captionof{table}{\\~\\Elenco di prezzi del titolo BITCOIN-EURO per i primi cinque minuti di gennaio 2020. Il primo scambio disponibile è stato effettuato alle 00:00:04 e, in totale, sono presenti 29 scambi.}\label{Tabella 0}
\newpage
\end{fig}
\begin{fig}
	\begin{center}
...
\end{tabular}
\end{center}
\captionof{table}{\\~\\Candela OHLCV risultante dal ricampionamento dei record nella Tabella precedente. Il timestamp della candela è quello dell'ultimo record e rappresenta tutti gli scambi avvenuti fino alle 05:00, partendo dall'ultima candela di cinque minuti.}\label{Tabella 1}
\end{fig}
...
%TODO: torna alla versione precedente dove c'era itemize e spiegavi le due differenti fasi di test, che ora ci sono.
...
%TODO: performance e' solo profit. Poi puoi spiegare che profit èdato da somma guadagni / somma perdite
%TODO: spiega valori che assume funzione?
\begin{split}
...
\begin{fig}
	...
	\end{center}
	\captionof{figure}{Grafico comparativo dei risultati prodotti da 10 differenti AI di Sentyment. In questo caso è analizzata la funzione \textit{performance} (Formula 6) per ciascuna delle AI prese in considerazione. La funzione è calcolata partendo dall'inizio del dataset (BITCOIN-EURO a partire da inizio 2014, fino ad inizio 2020) in intervalli di 250 candele orarie. Alcune AI si posizionano sempre in alto rispetto ad altre, ed è per questo motivo che è necessario sceglierne una migliore fra tutte.}\label{Figura 13}
\end{fig}
...
\begin{fig}
    \begin{center}
    	...
    	\end{tabular}
    \end{center}
	\captionof{table}{Elenco di record che rappresentano il risultato prodotto da una delle AI di Sentyment. Sono state considerate le candele orarie: per ogni ora si ha il segnale di buy / sell / hold. I trigger sono rappresentati da un valore numerico: 0 per hold, 1 per buy e -1 sell. In questa tabella è stata aperta una posizione di acquisto il 13 gennaio 2020 alle ore 18:00 ed è stato venduto il titolo alle 23:00}\label{Tabella 2}
\end{fig}
\\
\begin{fig}
	\\
	...
	\end{subfigure}
	\captionof{figure}{Esempio di grafico dei prezzi di chiusura di BITCOIN-EURO in un certo periodo, ed i \textit{trigger} di due AI di Sentyment. Si nota che i segnali buy / sell non corrispondono esattamente fra una AI e l'altra, confrontando questi due grafici, ma sono leggermente spostati. Ognuna produce strategie differenti e, anche se probabilmente molte AI identificano gli stessi trend nel grafico, alcune potrebbero decidere di comprare, o vendere, leggermente prima, o dopo, rispetto alle altre.}\label{Figura 14}
\\~\\
...
\begin{fig}
	...
	\end{tabular}
	\end{center}
	\captionof{table}{Elenco di record contenenti le scelte di alcune AI, unito ai prezzi di chiusura delle candele associate. Questi dataset sono tutto ciò che il supervisore necessita in input per essere allenato ed operare la sua scelta.}\label{Tabella 3}
\end{fig}
...
\begin{fig}
	...
	\end{center}
	\captionof{figure}{Idea generale di un processo di \textit{reinforcement learning}: un agente interagisce con l'ambiente per imparare dall'esperienza. Per ogni istante di tempo, \textit{t} l'agente è in un certo stato \textit{S\textsubscript{t}} ed esegue l'azione \textit{A\textsubscript{t}}. Come risultato, passa in un nuovo stato \textit{S\textsubscript{t+1}} e riceve una ricompensa \textit{R\textsubscript{t+1}}}\label{Figura 16}
\end{fig}
...
\begin{fig}
	...
	\end{center}
	\captionof{figure}{\\~\\Esempio di scelte effettuate durante il training di un agente multi-armed bandit. Al crescere del numero di scelte effettuate (Episode number), si avrà una concentrazione sempre maggiore della scelta su uno specifico bandit, quello che, secondo l'algoritmo, è il più promettente. In questo esempio, si può notare come il bandit preferito sia il numero 4. Già dopo 10\textsuperscript{2} scelte, i competitori si riducono praticamente a due soltanto, il bandit 4 ed il 9. Poco dopo, però, è il numero 4 ad essere scelto sempre più spesso nella fase di training.}\label{Figura 17}
\end{fig}
...
\\~\\ Alla fine di questa fase di training, l'agente fornisce l'elenco dello storico delle scelte effettuate. La AI migliore è quella scelta il maggior numero di volte rispetto a tutte le altre. Facendo riferimento alla Figura 18, alla fine del training vince il bandit che ha ricevuto più scelte da parte dell'agente quindi, in questo caso, Bandit #4.
...
\begin{fig}
	\begin{center}
		\begin{subfigure}{\linewidth}
			...
			\includegraphics[width=.5\linewidth]{bandit_choice_100}
		\end{subfigure}
		\begin{subfigure}{\linewidth}
			\includegraphics[width=\linewidth]{bandit_choice_1000}
		\end{subfigure}
	\end{center}
	\captionof{figure}{\\~\\Stesso grafico dei precedenti, scelte dell'algoritmo multi-armed bandit, questa volta al variare del numero di esperimenti. Si può notare una linea sempre meno spezzata al crescere dei punti, mentre con basso numero di esperimenti i punti sono molto pochi.}\label{Figura 23}
\end{fig}
...
\begin{fig}
	\begin{center}
		\includegraphics[width=\linewidth]{performance_ai_250}
	\end{center}
	\captionof{figure}{\\~\\Grafico riassuntivo delle performance delle 10 AI considerate per il training di \textit{meta-learner}. Le performance sono calcolate a step di 250 candele (circa 10 giorni) ed utilizzando la Formula (6). Le performance e i rendimenti sono fortemente correlate all'andamento dei prezzi. Questi sono i dati calcolati dal supervisore per il suo training: ha una ricompensa positiva se, all'interno di ogni step, sceglie la AI che ha un valore più alto di performance. Come si può notare, all'inizio, ci sono pochi picchi di performance e soprattutto da parte di AI\_0 (blu). Continuando più avanti nel tempo, i picchi compaiono sempre più spesso, sono più accentuati e sono prodotti da sempre più AI, risultato di un training di Sentyment che si adatta sempre di più ai dati e produce risultati sempre migliori.}\label{Figura 19}
\end{fig}
...
\begin{fig}
		...
		\end{subfigure}
	\captionof{figure}{\\~\\Performance di diverse AI a confronto, isolate dal grafico : la migliore (AI\_0), la seconda e terza posizione (AI\_8 e AI\_7), l'ultima (AI\_2) e la media di tutte.}\label{Figura 21}
\end{fig}
...
\begin{fig}
	...
	\end{center}
	\captionof{figure}{\\~\\Grafico ideale di una scelta perfetta. Se in ogni punto del  grafico delle performance (Figura 22) si fosse sempre scelta la AI con performance migliore, alla fine, si avrebbe una performance finale che è sempre il massimo fra le performance delle varie AI nel tempo. Sono stati scelti alcuni punti nel grafico precedente in cui scegliere la migliore AI e scambiare con la precedente migliore. Il risultato approssima la funzione di massimo, ma resta la questione di come scegliere una AI quando sta avendo un picco migliore delle altre.}\label{Figura 22}
\end{fig}
...
Confrontare diverse strategie è un compito non banale, vanno calcolati per ognuna molti indicatori e statistiche e si confrontano quelli che si ritengono più importanti a seconda della situazione e del tipo di dati. Difficilmente si avrà una strategia che è in assoluto migliore di tutte le altre, da tutti i punti di vista; si tratta più di imporre dei vincoli e operare delle scelte, sacrificando alcuni aspetti in favore di altri.\\ In questo ambito si vedrà un'analisi qualitativa e verranno esposti alcuni punti chiave che sono emersi studiando i grafici di profitto di varie strategie, ma non si intende in alcun modo comparare numericamente i risultati, in quanto strumenti più sofisticati e adatti allo scopo esistono già e un'analisi finanziaria dettagliata è al di fuori dell'ambito di questo documento.\\~\\Sentyment verrà ora trattata come un'unica AI, non più distinta in molti componenti differenti. Si suppone composta soltanto dalla AI che si è scelta come migliore nei passi precedenti e confrontata con altri strumenti simili.\\~\\ Il confronto risulta quindi difficile. I guadagni di ogni strategia sono fortemente correlati ai prezzi e quindi, in generale, se il mercato sta andando male difficilmente le strategie riescono ad avere un alto profitto. Inoltre, alcune strategie sono più adatte ad un certo tipo di tendenza dei prezzi e si rivelano inefficaci in qualsiasi altra situazione.\\ Dato che si parla di Bitcoin, si prenda come esempio una strategia buy-hold calcolata per l'intero periodo disponibile nel dataset (2014-2020), quindi analizzando sul lungo termine. Il titolo Bitcoin ha visto il suo prezzo aumentare sempre più, in una salita crescente fino a circa il 2018. In questa situazione, comprare un titolo a inizio 2014 e mantenerlo fino al 2018, senza effettuare nessun altra operazione, produce un rendimento altissimo e non c'è strategia che riesca a batterlo. Un asset comprato in precedenza per una certa quantità, ora ne vale almeno 100 volte tanto. Per questi motivi, sul lungo periodo e in una situazione di crescita costante, nonostante eventuali massimi e minimi locali, una strategia di tipo buy-hold risulta molto redditizia e adatta alla situazione.
\begin{fig}
	\begin{center}
		\includegraphics[width=\linewidth]{final_all}
	\end{center}
	\captionof{figure}{\\Rendimento di varie strategie applicate circa all'intero dataset  Bitcoin-Euro. Si nota che buy-hold sovrasta tutte le altre, perchè è una strategia adatta a questa situazione, in cui il valore di Bitcoin ha delle crescite di prezzo altissime. Le altre strategie elencate sono per le analisi a breve termine.}\label{Figura 24}
\end{fig}
...
\begin{fig}
	\begin{subfigure}{\linewidth}
		\includegraphics[width=.5\linewidth]{final_example_1}
		\includegraphics[width=.5\linewidth]{final_example_2}
	\end{subfigure}
	\begin{subfigure}{\linewidth}
		\includegraphics[width=.5\linewidth]{final_example_3}
		\includegraphics[width=.5\linewidth]{final_example_2017}
	\end{subfigure}
	\captionof{figure}{\\Analisi di un breve periodo (dieci giorni) e un alcuni medi periodi di qualche mese di lunghezza. Si notano due scenari differenti, in cui nel primo Sentyment è comparabile ad altre strategie. Alcune strategie falliscono in certi tipi di situazioni, come MACD che vede il suo rendimento crollare nella seconda immagine. Spesso quando si parte con un'impennata nei prezzi, buy-hold risulta la migliore. Quando invece si parte con un calo o si verifica una decrescita nel periodo, Sentyment reagisce meglio di altre.}\label{Figura 25}
\end{fig}
...
\begin{fig}
	...
	\end{subfigure}
	\captionof{figure}{\\Grafici di profitto di due particolari periodi per Bitcoin. Il primo, relativo al picco di prezzi di fine 2017 / inizio 2018, vede buy-hold in prima posizione, grazie al cresente valore del titolo. Sentyment vende spesso al post di mantenere e perde parte del profitto. Nella seconda immagine si vede che, però, una volta finito il picco ascendente, quando i prezzi sono calati e stanno tornando piatti o oscillanti, Sentyment ritorna in prima posizione.}\label{Figura 26}
\end{fig}
\\~\\Per quanto riguarda un'analisi di tipo più quantitativo, è possibile calcolare, per ognuno dei periodi in cui è stato diviso il dataset, diverse statistiche per poterle confrontare con quelle prodotte dalle altre strategie, sempre all'interno dei periodi. È possibile analizzare sotto diversi aspetti e gli indicatori sono molti e ognuno esprime una qualità differente.\\ Al fine di questa analisi, che non ha la pretesa di essere un'analisi finanziaria completa sulle strategie di Sentyment ma soltanto una discussione sulle sue performance, sono trattate le metriche già esposte, quindi rendimento e profitto. Il rendimento, qua chiamato anche come "guadagno", esprime l'andamento della strategia rispetto ai prezzi mentre il profitto pesa anche il fattore perdite ed esprime quindi anche un fattore di rischio. Non è esattamente la Formula (6) usata nel \textit{meta-learner}, perchè pesava le perdite più delle vincite; qua invece sono usati entrambi i pesi a 1. Purtroppo, però, non è possibile calcolare la performance per buy-hold o per periodi troppo brevi (come già trattato in questo capitolo), in quanto basa le sue statistiche sull'analisi delle singole operazioni, vincenti e non, mentre buy hold non esegue mai alcuna operazione, ad eccezione delle due iniziali e finali. Buy hold sarà quindi esclusa da questo paragone.\\~\\
...
\begin{fig}
	...
	\end{subfigure}
	\captionof{figure}{\\Analisi di rendimento e performance in brevi periodi da 100 e 250 candele. I grafici sono simili a quelli usati da \textit{meta-learner} in cui si analizzano le performance, ma qui le perdite sono pesate allo stesso modo delle vincite, risultando in performance medie più alte. I profitti di Sentyment sono inversamente proprorzionali al periodo di analisi. Sentyment compete con RSI in 100 / 250 candele. La performance di buy-hold non può essere calcolata in quanto necessiterebbero molte operazioni di buy / sell.}\label{Figura 27}
\end{fig}
...
\begin{fig}
	\begin{subfigure}{\linewidth}
		...
		\end{tabular}
	\end{subfigure}
	\captionof{table}{\\~\\Rendimenti e performance medie delle strategie considerate per il confronto, relative a periodi di 100 e 250 candele.}\label{Tabella 4}
\end{fig}
...
Siccome complessità è alta, o comunque non lineare, è difficile da usare per periodi grandi. Anche per brevi periodi è comunque scomodo. meglio usare un'approssimazione che, anche se non arriva agli stessi numeri, è comunque ben oltre qualsiasi strategia e rappresenta comunque un modello a cui rifarsi; inoltre è velocissima rispetto all'algoritmo.

E poi funziona bene perchè è possibile che combinazioni completamente diverse di segnali producano comunque un alto guadagno, molto superiore a quello prodotto da qualsiasi altra strategia e magari anche comparabile con il maximum profit. Quindi già avere un modello semi-ottimo potrebbe portare dei miglioramenti nello sviluppo e nei test, cosa che maximum profit invece potrebbe fare fatica ad apportare, vista la lentezza con cui esegue.

Infatti il secondo grafico in figura 36 è stato prodotto con genetic e non maximum profit, perchè avrebbe richiesto troppo tempo e comunque si nota che non in un singolo punto altre strategie sorpassano in performance genetic.



\subsubsection{Risultati}
TABELLA DI PRIMA CON AGGIUNTO MAX PROF,
PER PERIODI DA 100 250 E ANCHE 50K

Mean profit with period 45584
buy hold: 97.85603800490317
macd: -8.894687747064413
rsi: -2.7155031004831307
sentyment: -6.0521396792777695
max profit: 4395810751585198.5



Hai scoperto che è meglio il lungo periodo? È soltanto perchè siccome bitcoin era in crescita, e l'algoritmo lo sapeva, allora fa indubbiamente guadagnare di più in assoluto rispetto ad altri mercati. Andrebbe fatta la prova in ogni moneta per confermare che la strategia dipende dalla situazione, ma tanto è inutile perchè nessuno può conoscere la situazione in anticipo, i trend a lungo termine.
Meglio sempre lavorare sul breve termine e sfruttare i brevi picchi, tanto anche lì c'è tanto da fare e il guadagno è possibile.

Guarda anche NOTE per altro.
