...
\item nel Capitolo 4 è analizzata la AI in questione, è sviluppato un nuovo strumento intelligente che sceglie i migliori parametri per Sentyment e questa è infine confrontata con altri mezzi simili.
\item il Capitolo 5 conclude i test e offre spunti per eventuali futuri miglioramenti.
\end{itemize}
%
%			RINGRAZIAMENTI
%
...
\\~\\
Questa formula viene riutilizzata nel Capitolo 4 nell'ambito dello sviluppo degli strumenti di test. Da notare che il valore complessivo potrebbe scendere sotto il budget iniziale e in questo caso si dovrebbe parlare di \textit{perdita}, ma è comunque usato il termine \textit{guadagno} per semplicità.\\~\\
...
\begin{fig}
	...
	\end{center}
	\captionof{figure}{Esempio dei \textit{trigger} generati da una delle AI di Sentyment. Il grafico fa riferimento ai prezzi di chiusura del ticker BITCOIN-EURO relativi ad un certo periodo temporale, da fine 2019 a inizio 2020.}\label{Figura 11}
\end{fig}
...
\begin{fig}
	\begin{center}
		\includegraphics[width=\linewidth]{prices_rsi}
	\end{center}
\end{fig}
\begin{fig}
	\begin{center}
		\includegraphics[width=\linewidth]{rsi_rsi}
	\end{center}
	\captionof{figure}{Grafico dei prezzi di chiusura, corredato di segnali buy e sell generati dalla strategia RSI. Nella seconda immagine è mostrato l'andamento del valore dell'indicatore RSI e i punti in cui tocca gli estremi 30 e 70, i quali indicano di generare il segnale compra e vendi.}\label{Figura 30}
\end{fig}
\\~\\Una volta calcolata la funzione e i punti di crossover, si ottiene un dataset di prezzi accompagnato dai trigger. Quelli mostrati in figura sono soltanto relativi ai buy e sell; in tutti gli altri punti è segnata un'operazione di hold (0). Per ogni istante si ha quindi l'operazione buy / sell / hold (Tabella 3) e questo è tutto ciò che serve ai passi successivi per le analisi, infatti nel Capitolo 4 si useranno questi record per comparare varie strategie fra di loro, trattando indifferentemente quelle prodotte da indicatori, come RSI, e quelle generate da una AI. Anche le AI di Sentyment, infatti, producono un elenco di record di questo tipo.\\
\begin{fig}
	\begin{center}
		\begin{tabular}{||c c ||} 
			\hline
		...
		\end{tabular}
	\end{center}
	\captionof{table}{Elenco di trigger che rappresentano la strategi RSI, ricavati dai crossover dell'indicatore RSI del grafico precedente.}\label{Tabella 9}
\end{fig}
...
\end{fig}
\\~\\Nelle successive immagini è analizzata la funzione di performance di AI\_0, isolandola dalle altre in Figura 23. Si può notare che la performance di AI\_0 è quasi sempre superiore di tutte le altre, considerando picco per picco. 
\begin{fig}
		\begin{subfigure}{\linewidth}
...
\begin{fig}
	\begin{subfigure}{\linewidth}
		...
		\end{tabular}
	\end{subfigure}
	\captionof{table}{\\~\\Rendimenti medi di alcune strategie e di maximum profit, relative a periodi di 100 e 250 candele e su tutto il periodo 2014-2020.}\label{Tabella 5}
\end{fig}
