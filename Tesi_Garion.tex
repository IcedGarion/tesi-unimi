...
\begin{fig}
	...
	\end{center}
	\captionof{figure}{Grafico comparativo dei risultati prodotti da 10 differenti AI di Sentyment. In questo caso è analizzata la funzione \textit{performance} (Formula 7) per ciascuna delle AI prese in considerazione. La funzione è calcolata partendo dall'inizio del dataset (BITCOIN-EURO a partire da inizio 2014, fino ad inizio 2020) in intervalli di 250 candele orarie. Alcune AI si posizionano sempre in alto rispetto ad altre, ed è per questo motivo che è necessario sceglierne una migliore fra tutte.}\label{Figura 13}
\end{fig}
...
\begin{fig}
	...
	\end{subfigure}
	\captionof{figure}{\\Grafici di profitto di due particolari periodi per Bitcoin. Il primo, relativo al picco di prezzi di fine 2017 / inizio 2018, vede buy-hold in prima posizione, grazie al cresente valore del titolo. Sentyment vende spesso al posto di mantenere e perde parte del profitto (probabilmente non sta cercando di identificare il trend globale). Nella seconda immagine si vede che, però, una volta finito il picco ascendente, quando i prezzi sono calati e stanno tornando piatti o oscillanti, Sentyment ritorna in prima posizione.}\label{Figura 26}
\end{fig}
