...
%TODO: da sistemare se aggiungi buy-hold macd e genetic
...
A scopo comparativo, avendo a disposizione un dataset di prezzi, è possibile calcolare quali sarebbero state le scelte da compiere per produrre il \textit{massimo guadagno assoluto}. Esiste una precisa combinazione di segnali buy / sell per ogni intervallo di prezzi che, se attuata con un certo budget iniziale e delle fee, produce un rendimento massimo rispetto a qualsiasi altra possibile combinazione di segnali a partire dagli stessi dati.\\ Questa tecnica non anticipa i prezzi nè si può dire una vera e propria strategia, perchè non esprime delle regole secondo cui comprare o vendere a seconda dell'andamento dei prezzi, ma è soltanto ricavabile da prezzi passati ed è un'ulteriore base di confronto per altre strategie.\\~\\ L'algoritmo è assai lontano dall'essere ottimo, infatti ha una complessità teorica di O(k*n\textsuperscript{2}) nel numero degli elementi (n), cioè la lista di prezzi e il numero massimo di operazioni di compravendita (k, che sarà sempre n/2). Esiste però una versione ottimizzata che fa uso di tecniche di programmazione lineare, riutilizzando alcuni casi già calcolati, che aiuta a ridurre la complessità a O(k*n). Per semplicità è qua riportato solo un esempio della versione non ottimizzata e senza fee.


Descrivi algoritmo facile senza fee
Descrivi adattamento a fee (non piu' sempre in salita perche' quando compri primo titolo o vendi (in generale) abbassi il rendimento pagando la fee)
Descrivi complessità e quant'altro, 
Non è fattibile su tutto dataset ma solo su pezzi per capire andamento strategia e avere un riferimento assoluto.
...
\begin{fig}
	...
	\end{subfigure}
	\captionof{figure}{\\Immagini che mostrano l'efficacia dell'algoritmo maximum profit. È mostrata prima la sezione di un breve periodo di qualche giorno e poi l'intero dataset suddiviso in candele da 100 ore. Per ciascuno dei due grafici è mostrato l'andamento del rendimento ed entrambi sono analizzati con le stesse strategie utilizzate per i confronti precedenti. Si può notare come la "strategia" prodotta da maximum profit ha rendimenti nettamnte superiori rispetto a qualsiasi altra, ma è un algoritmo teorico e non realizzabile nella pratica perchè necessita di conoscere in anticipo i prezzi.}\label{Figura 28}
\end{fig}

Come si nota dalla prima immagine, in presenza di fee la funzion non è più monotona non decrescente, ma presenta delle perdite. Questo è dovuto alle fee. Quando si esegue un'operazione di acquisto o di vendita, il valore dei titoli posseduti (unito al budget) decresce per un momento di una minima percentuale dovuta alla tassa. L'algoritmo di maximum profit va adattato alla fee, ma dal punto di vista concettuale continua a funzionare: esprime comunque la miglior combinazione possibile di operazioni da eseguire per avere massimo rendimento.

Puo' essere approssimato abbastanza bene con una genetic AI.
